...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text)
{
    ...
    m_url = strpbrk(text, " \t");//返回值为 char*,为\t的位置
    ...
    *m_url++ = '\0';//将该位置改为\0，用于将前面数据取出
    ...
    if (strcasecmp(method, "GET") == 0)//strcasemcmp函数的字符串参数应包含一个(’\0’)标记字符串结尾的空字符
        m_method = GET;
    ...
    *m_version++ = '\0';
    ...
    if (strncasecmp(m_url, "http://", 7) == 0)//m_url, "http://"相同返回0
    {
        m_url += 7;
        m_url = strchr(m_url, '/');
    }
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    //printf("m_url:%s\n", m_url);
    const char *p = strrchr(m_url, '/');//返回最后一个出现/的位置,找到m_url中/的位置
